...
# Check if script is installed as cron job
DAEMON_INSTALLED=$(cru l | grep UpdateTrafficDBs)
...
update_web_db() {
        if [ -z "${DAEMON_INSTALLED}" ]; then
                ...
                WebHistory -e
                echo "Log: DB size is $(get_web_db_size) MB"
        else
                echo "Error: daemon is installed"
                exit 3
        fi
}
...
update_traffic_db() {
        if [ -z "${DAEMON_INSTALLED}" ]; then
                ...
                END_SQL
                echo "Log: Updating IP Traffic database"
                TrafficAnalyzer -e
                echo "Log: DB size is $(get_traffic_db_size) MB"
        else
                echo "Error: daemon is installed"
                exit 3
        fi
